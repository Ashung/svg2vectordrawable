    ...
    .act(function(opts, args) {
        ...
        if (opts.string) {
            ...
            if (svgFiles.length === 1 && !/\.xml$/i.test(opts.output)) {
                return showErrorAndExit('Output must be a XML file.');
            }
            if (svgFiles.length > 1 && /\.[0-9a-z]+$/i.test(opts.output)) {
                return showErrorAndExit('Output must be a folder.');
            }

            svgFiles.forEach(svgFile => {
                let filePath = opts.output;
                if (!/\.xml$/i.test(opts.output)) {
                    let fixedFileName =  path.basename(svgFile, '.svg').replace(/[^a-z0-9]/gi, '_').replace(/^\d+/,'').toLowerCase();
                    filePath = path.join(filePath, fixedFileName + '.xml');
                }
                return convertFile(svgFile, filePath).then(() => {
