...
const svgoConfig = require('./svgo_config');
...
JS2XML.prototype.refactorData = function(data, floatPrecision) {
    ...
    svgoUsePlugins(data, svgoConfig.info, config(svgoConfig(floatPrecision)).plugins);

    // SVGO do not apply transform to path that attribute value is url(). svgo/plugins/_path.js
    // and Android vector drawable not support transform in path.
