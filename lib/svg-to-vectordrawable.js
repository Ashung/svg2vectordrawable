...
const svgo = require('./svgo').svgOptimize;
const svgoInfo = require('./svgo').info;
...
module.exports = function(svgCode, floatPrecision) {
    ...
    return new Promise((resolve, reject) => {
        svgo(svgCode, floatPrecision).then(svgOptimized => {
            ...
            svg2js(svgOptimized, result => {
                ...
                if (result.err) {
                    ...
                    let xml = new JS2XML().convert(result, floatPrecision);
                    resolve(xml);
                }
            });
